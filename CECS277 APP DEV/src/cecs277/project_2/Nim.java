...
public class Nim extends ARepl {
	...
	public void brainMove()
	{
		//Checking to make sure the computer picks an appropriate random number
		//for both the row and the number of stones
		if (board[move.getP2Row() - 1] >= move.getP2Stone())
		{
			//Printing the board
			displayGameBoard();
			System.out.println("\nBrain, your turn.");
			System.out.println("Brain removes " + move.getP2Stone() + " stone(s) from row " + move.getP2Row());
			board[move.getP2Row() - 1] -= move.getP2Stone();
			turn++;
		}
	}
	...
	public void hello() 
	{
		...
		System.out.println("The goal is to be the last person to clear all stones");
		System.out.println("from every row.");
